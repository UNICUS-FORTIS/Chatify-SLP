...
final class ValidationCenter {
    ...
    func validateWorkspaceName(_ nickname: String) -> Bool {
        let workspacenameRegex = "^(?=.*[가-힣A-Za-z])[가-힣A-Za-z0-9\\s]{1,30}$"
        let nickNameTest = NSPredicate(format:"SELF MATCHES %@", workspacenameRegex)
        return nickNameTest.evaluate(with: nickname)
    }
